...
const sendMessage = () => {
  if (message.value !== null && message.value !== '') {
    Inertia.post(route('enquiry.chatroom.createnew', { 'message': message.value, 'sale': props.sale }))
  }
}
const uploadFile = () => {
...
<template>
  <ChatRoomLayout title="Chatroom" :showInfo=false :navigationBack="'/chats/enquiry/asdf'"
    :name="`My Enquiry - ${props.sale.product_name} - ${props.sale.cas_no} - Initiated By ${$page.props.user.name} - Posted By ${props.sale.user.name}`"
