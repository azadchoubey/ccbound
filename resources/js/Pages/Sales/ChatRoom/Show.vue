...
<template>
  <ChatRoomLayout title="Chatroom" :showInfo=true :navigationBack="'/chats/enquiry/asdf'"
    :infoLink="route('sale.chatroom.settings', { 'chatroom': $props.chatroom })"
      ...
      <div id="container" class="px-2 flex-1 mb-[5rem] w-full bg-white h-[calc(100vh_-_150px)] overflow-y-auto">
        <div class="w-full p-1">
          <div class="" v-for="message in chatroomMessages">
            <SentMessageCard v-if="message.user && message.user.id === $page.props.user.id" :message="message" :name="message.user.name"
              :company="message.user.company.name" :profile_image="message.user.profile_photo_url"
              :time="message.created_at" @messageDeleted="handleMessageDeleted" />

            <ReceivedMessageCard v-else :message="message" :name="message?.user?.name" :company="message?.user?.company.name"
        ...
        <div class="p-2 px-3 bg-white">
          <div v-if="auth_member.pivot.chat_left == 0" class="flex">
            <div class="flex items-center flex-1 px-3 py-1 bg-gray-200 rounded-3xl">
              ...
              <input type="file" class="hidden" id="file" @input="form.file = $event.target.files[0]"
