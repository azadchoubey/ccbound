...
const getChatrooms = (page = 1) => {
  axios.get(route('sales.chats.show', { page: page, tab: tab.value, chat: props.chat.id })).then(res => {
...
watch(search, async (newSearchQuery, oldSearchQuery) => {
  if (newSearchQuery.value !== '') {
    axios.get(route('sale.chats.show', { tab: tab.value, chat: props.chat.id, search: newSearchQuery })).then(res => {
      chatroomList.value = res.data
    })
  } else {
...
watch(tab, async (newSearchQuery, oldSearchQuery) => {
  axios.get(route('sale.chats.show', { tab: tab.value, chat: props.chat.id })).then(res => {
    chatroomList.value = res.data
  })
})

const addStar = (id) => {
  axios.post(route('sale.chats.star', { chat: props.chat, chatroom: id }))
    .then(res => {
...
const chatProductName = (title) => {
  // console.log(props.chat.product_name+'-'+props.chat.cas_no+' - ');
  let titleWithoutSpaces = title.replace(/\s+/g, '');

  return titleWithoutSpaces.replace(props.chat.product_name.replace(/\s+/g, '') + '-' + props.chat.cas_no.replace(/\s+/g, '') + '-', ' ').trim();
}
    ...
    <div class="max-w-[45rem]">
      <div class="flex flex-col px-2 bg-white">
        <div class="bg-white p-[1rem]">
          <p class="text-xl font-bold">My Sale</p>
        ...
        <div class="mt-4">
          ...
          <div v-for="(chatroom) in chatroomList.data" class="mt-4"
            ...
            <div class="flex items-center w-full gap-4 p-1 rounded-lg">
              ...
              <Link :href="route('sale.chatroom.show', { chatroom: chatroom })" class="w-full py-2">
              <div class="flex items-center justify-between w-full">
                <div>
                  ...
                  <p :class="`text-sm ${chatroom.message_read !== true ? 'font-bold' : ''}`">
                    <span class="text-blue-400" v-if="chatroom.last_message && chatroom.last_message.user">
